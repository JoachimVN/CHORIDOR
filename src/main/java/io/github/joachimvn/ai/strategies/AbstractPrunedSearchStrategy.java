...
/**
 * Wall-pruned iterative-deepening minimax shared by Tactical and Rusher.
 * Subclasses supply only the non-terminal heuristic via {@link #score(int, int)}.
 */
abstract class AbstractPrunedSearchStrategy implements Strategy {

    // Only consider walls within this Chebyshev distance of either pawn.
    // Reduces branching factor from ~130 to ~20-30, allowing much deeper search.
    ...
    /**
     * Combine the two BFS goal distances into a non-terminal score.
     * Higher favours {@code aiPlayer}. Called only for non-terminal positions.
     */
    ...
    private Move searchDepth(GameState state, List<Move> moves, int depth) {
        ...
        for (Move move : moves) {
            if (System.currentTimeMillis() >= deadline) break;
            int s = minimax(apply(state, move), depth - 1, Integer.MIN_VALUE, Integer.MAX_VALUE, false);
            if (s > bestScore) {
                bestScore = s;
                best = move;
            }
