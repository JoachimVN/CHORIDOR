...
/** Builds the CHORIDOR word-mark from its SVG paths, either scale-bound (top bar) or fixed-size (setup). */
public final class LogoFactory {

    private LogoFactory() {}

    /** Logo whose size tracks {@code scaleB}; used in the top chrome bar. */
    public static Pane scaling(DoubleBinding scaleB) {
        Group group = group();

        double baseLogoWidth = UiConstants.SVG_WIDTH * UiConstants.LOGO_TARGET_HEIGHT / UiConstants.SVG_HEIGHT;
        Scale scaleTransform = new Scale();
        scaleTransform.xProperty().bind(scaleB.multiply(UiConstants.LOGO_TARGET_HEIGHT / UiConstants.SVG_HEIGHT));
        scaleTransform.yProperty().bind(scaleB.multiply(UiConstants.LOGO_TARGET_HEIGHT / UiConstants.SVG_HEIGHT));
        group.getTransforms().add(scaleTransform);

        Pane pane = new Pane(group);
        pane.prefWidthProperty().bind(scaleB.multiply(baseLogoWidth));
        pane.prefHeightProperty().bind(scaleB.multiply(UiConstants.LOGO_TARGET_HEIGHT));
    ...
    /** Logo at a fixed pixel height; used as the setup-overlay title. */
    public static Pane fixed(double targetHeight) {
        Group group = group();
        double scale = targetHeight / UiConstants.SVG_HEIGHT;
        group.getTransforms().add(new Scale(scale, scale));
        double w = UiConstants.SVG_WIDTH * scale;
        Pane pane = new Pane(group);
    ...
    private static Group group() {
        ...
        for (String[] entry : paths()) {
            ...
            p.setFill("s1".equals(entry[1]) ? UiConstants.LOGO_BLUE : UiConstants.LOGO_RED);
